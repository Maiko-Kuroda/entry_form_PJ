    ...
    <div class="row">
        <div class="col-md-8 mx-auto">
            ...
            <form action="{{action('Admin\QuestionaryController@showDetail')}}" method="get">
                ...
                <input type="hidden" name="q5" value="{{ $user->questionaries->q5 }}">
                <input type="hidden" name="q6" value="{{ $user->questionaries->q6 }}">
                <input type="hidden" name="q7" value="{{ $user->questionaries->q7 }}">
                <input type="hidden" name="q8" value="{{ $user->questionaries->q8 }}">
                <input type="hidden" name="q9" value="{{ $user->questionaries->q9 }}">
                <input type="hidden" name="q10" value="{{ $user->questionaries->q10 }}">
                <input type="hidden" name="q11" value="{{ $user->questionaries->q11 }}">
                <input type="hidden" name="q12" value="{{ $user->questionaries->q12 }}">
                <input type="hidden" name="q13" value="{{ $user->questionaries->q13 }}">
                <input type="hidden" name="q14" value="{{ $user->questionaries->q14 }}">
                <input type="hidden" name="q15" value="{{ $user->questionaries->q15 }}">
                ...
                <div class="questionary-form-group ml-5">
                    ...
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q1)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>B_せき、のどの痛みなど、風邪の症状はありません</label>
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q2)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>C_ だるさ、息苦しさはありません</label>
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q3)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>D_嗅覚や味覚の異常はありません</label>
                        <div class="form-group">
                            <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q4)) !!}</p>
                        </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>E_体が重く感じる、疲れやすい等の症状はありません</label>
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q5)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>F_新型コロナウイルス感染症陽性とされた者と濃厚接触はありません</label>
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q6)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>G_同居家族や身近な知人に感染が疑われる方はいません</label>
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q7)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group ml-5">
                    <label>H_過去14日以内に政府から入国制限、入国後の観察期間を必要とされている国、<br>地域と等への渡航又は当該在住者との濃厚接触はありません</label>
                    <div class="form-group">
                        <p class="confirm-input ml-3 font-weight-bold">{!! nl2br(e($user->questionaries->q8)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group mb-3">
                    ...
                    <div class="form-group">
                        <p class="confirm-input ml-5 font-weight-bold">{!! nl2br(e($user->questionaries->q9)) !!}</p>
                    </div>
                </div>
                <div class="questionary-form-group mb-3">
